...
export default function TextLink({ href, children, className = '', external }: TextLinkProps) {
  ...
  const content = (
    <>
      ...
      <span className="absolute bottom-[-4px] left-0 h-[2px] w-full scale-x-0 bg-current motion-safe:transition-transform motion-safe:duration-[var(--motion-duration-standard)] motion-safe:ease-[var(--motion-ease-standard)] origin-right group-hover:scale-x-100 group-hover:origin-left" />
    </>
  );
